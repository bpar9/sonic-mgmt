...
def tgen_preconfig(**kwargs):
    ...
    if l3_cross_dc_endpoints:
        l3_cross_combos = set((ep.get('src_vrf'), ep.get('src_vlan')) for ep in l3_cross_dc_endpoints.values())
        st.log(f"  L3 cross-DC (DCI): {len(l3_cross_dc_endpoints)} endpoints, {len(l3_cross_combos)} VRF-VLAN combos")
        l3_cross_v4 = vxlan_obj.create_traffic_item(device_handles=v4_device_handles,
                                                    endpoints=l3_cross_dc_endpoints,
                                                    topo_handles=topo_handles,
                                                    multi_dst='vrf', name_prfx='L3-DCI-CROSS',
                                                    rate_percent=test_cfg['global'].get('l2l3', {}).get('rate_percent', 0.8),
                                                    pkts_per_burst=test_cfg['global'].get('l2l3', {}).get('pkts_per_burst', 1000))
        if l3_cross_v4:
            stream_handles['l3_v4'].extend(l3_cross_v4 if isinstance(l3_cross_v4, list) else [l3_cross_v4])
    ...
    if l3_cross_dc_endpoints:
        l3_cross_v6 = vxlan_obj.create_traffic_item(device_handles=v6_device_handles,
                                                    endpoints=l3_cross_dc_endpoints,
                                                    topo_handles=topo_handles,
                                                    version="ipv6", multi_dst='vrf', name_prfx='L3-DCI-CROSS',
                                                    rate_percent=test_cfg['global'].get('l2l3', {}).get('rate_percent', 0.8),
                                                    pkts_per_burst=test_cfg['global'].get('l2l3', {}).get('pkts_per_burst', 1000))
        if l3_cross_v6:
            stream_handles['l3_v6'].extend(l3_cross_v6 if isinstance(l3_cross_v6, list) else [l3_cross_v6])
...
class TestVxlanDCIBase():
    ...
    def test_base_dci_l3vni_base_profile(self):
        """
        L3VNI_dci:1 - L3VNI Base Profile - Bring up L3VNI base profile across DCI
        
        Description:
            1) Verify L3VNI mapping (show vxlan vlanvnimap)
            2) Verify VRF to VNI binding (show vxlan vrfvnimap)
            ...
            3) Verify Type-5 routes advertised (show bgp l2vpn evpn route type prefix)
        ...
        Steps:
            1. Verify VRF-VNI, VLAN-VNI mappings and Type-5 routes via verify_base_setup_bgw
        """
        ...
        # Step 1: Verify VRF-VNI, VLAN-VNI mappings and Type-5 routes using verify_base_setup_bgw
        st.banner('Step 1: Verify VRF-VNI, VLAN-VNI and Type-5 routes on all DCI nodes')
    ...
    def test_base_dci_l3vni_type5_route_ipv6_vtep(self):
        """
        L3VNI_dci:2 - L3VNI Control Plane - Type-5 route advertisement within DC (IPv6 VTEP)
    ...
    def test_base_dci_l3vni_type5_ipv6_prefix(self):
        ...
        # Step 1: Verify Type-5 routes on all nodes (leaf + BGW)
        st.banner('Step 1: Verify Type-5 routes for IPv6 prefixes on all nodes (leaf + BGW)')
        if not verify_base_setup_bgw(test_cfg['nodes']['l2l3vni_bgw'],
                                     checks=['evpn_type5_comprehensive']):
            summ += 'Type-5 route verification failed on leaf or BGW nodes\n'
            result = False
    ...
    def test_base_dci_l3vni_type5_ipv4_prefix(self):
        ...
        # Step 1: Verify Type-5 routes on all nodes (leaf + BGW)
        st.banner('Step 1: Verify Type-5 routes for IPv4 prefixes on all nodes (leaf + BGW)')
        if not verify_base_setup_bgw(test_cfg['nodes']['l2l3vni_bgw'],
                                     checks=['evpn_type5_comprehensive']):
            summ += 'Type-5 route verification failed on leaf or BGW nodes\n'
            result = False
